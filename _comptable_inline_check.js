            ...
            const trashRef = db.ref('trash');

            // Corbeille centrale : toute suppression de donnée métier passe d'abord ici.
            // La donnée reste restaurable jusqu'à une suppression définitive depuis la Corbeille.
            async function moveToTrash(ref, recordId, options = {}) {
                if (!ref || !recordId) throw new Error('Identifiant de donnée manquant pour la corbeille.');
                const snap = await ref.child(recordId).once('value');
                if (!snap.exists()) return false;
                const trashItem = {
                    originalPath: `${ref.toString().split('/').slice(-1)[0]}/${recordId}`,
                    originalId: String(recordId),
                    originalData: snap.val(),
                    deletedAt: Date.now(),
                    deletedBy: currentUser?.email || currentUser?.name || 'System',
                    deletedByUid: currentUserUID || '',
                    reason: options.reason || 'Suppression manuelle',
                    source: options.source || 'system',
                    label: options.label || snap.val()?.name || snap.val()?.studentName || snap.val()?.reference || String(recordId)
                };
                const trashItemRef = trashRef.push();
                await trashItemRef.set(trashItem);
                await ref.child(recordId).remove();
                return true;
            }

            async function restoreFromTrash(trashId) {
                const snap = await trashRef.child(trashId).once('value');
                if (!snap.exists()) throw new Error('Élément de corbeille introuvable.');
                const item = snap.val() || {};
                const path = (item.originalPath || '').split('/').filter(Boolean);
                if (path.length < 2) throw new Error('Chemin original invalide.');
                const originalRef = db.ref(path[0]);
                await originalRef.child(path.slice(1).join('/')).set(item.originalData);
            ...
            async function permanentlyDeleteFromTrash(trashId) {
                ...
                const path=(item.originalPath||'').split('/').filter(Boolean);
                if(path.length>=2){try{await db.ref(path[0]).child(path.slice(1).join('/')).remove();}catch(e){console.warn('Original Firebase record already absent or inaccessible:',e);}}
                await trashRef.child(trashId).remove();
            }
            ...
            async function emptyTrash() {
                ...
                if (!confirm(`Vider définitivement la corbeille (${count} élément(s)) ? Cette action est irréversible.`)) return;
                const removals=[]; snap.forEach(child=>{const item=child.val()||{};const path=(item.originalPath||'').split('/').filter(Boolean);if(path.length>=2)removals.push(db.ref(path[0]).child(path.slice(1).join('/')).remove().catch(()=>null));});
            ...
            async function loadTrash() {
                ...
                if (!body) return;
                body.innerHTML = '<tr><td colspan="8" class="text-center py-4"><i class="fas fa-spinner fa-spin me-2"></i>Chargement de la corbeille...</td></tr>';
                try {
                    const snap = await trashRef.once('value');
                    const rows = [];
                    ...
                    if (!rows.length) {
                        body.innerHTML = '<tr><td colspan="8" class="text-center text-muted py-5"><i class="fas fa-trash-alt fa-2x mb-2"></i><br>La corbeille est vide.</td></tr>';
                        return;
                    }
                    body.innerHTML = rows.map(item => {
                        const data = item.originalData || {};
                        const label = escapeDuplicateHtml(item.label || data.name || data.studentName || data.reference || item.originalId);
                        const source = escapeDuplicateHtml(item.source || 'system');
                        const deleted = item.deletedAt ? new Date(item.deletedAt).toLocaleString('fr-FR') : '—';
                        return `<tr><td><strong>${label}</strong></td><td>${source}</td><td>${escapeDuplicateHtml(item.originalPath || '')}</td><td>${deleted}</td><td>${escapeDuplicateHtml(item.deletedBy || '')}</td><td>${escapeDuplicateHtml(item.reason || '')}</td><td><button class="btn btn-sm btn-outline-success me-1" onclick="restoreFromTrash('${item.id}').then(loadTrash).catch(e=>showToast(e.message,'error'))"><i class="fas fa-undo"></i> Restaurer</button><button class="btn btn-sm btn-outline-danger" onclick="permanentlyDeleteFromTrash('${item.id}').then(loadTrash).catch(e=>showToast(e.message,'error'))"><i class="fas fa-trash"></i> Définitif</button></td></tr>`;
                    }).join('');
                } catch (error) {
                    console.error('Erreur chargement corbeille:', error);
                    body.innerHTML = '<tr><td colspan="8" class="text-center text-danger py-4">Impossible de charger la corbeille.</td></tr>';
                }
            }
            ...
            async function deleteTransportPayment(paymentId) {
                ...
                try {
                    await moveToTrash(transportPaymentsRef, paymentId, {source:'transport', reason:'Suppression d’un paiement de transport'});
                    showToast('Transport payment moved to trash.', 'success');
                    loadTransport();
                    recalculateExpectedIncome();
                } catch (error) {
            ...
            function comptableNormalizeAcademicYear(value) {
                const raw=String(value??'').trim().replace(/[â€“â€”âˆ’\/]/g,'-');
                const m=raw.match(/(20\d{2})\s*-\s*(20\d{2}|\d{2})/);
                if(!m)return '';
                return `${m[1]}-${m[2].length===2?String(Number(m[1])+1):m[2]}`;
            }
            function comptableActiveAcademicYear() {
                const saved=comptableNormalizeAcademicYear(localStorage.getItem('eden_active_school_year'));
                if(saved)return saved;
                const now=new Date(); const y=now.getFullYear(); return `${now.getMonth()>=8?y:y-1}-${now.getMonth()>=8?y+1:y}`;
            }
            function comptableRecordYear(record, student) {
                return comptableNormalizeAcademicYear(record?.academicYear||record?.anneeAcademique||record?.anneeScolaire||record?.schoolYear||student?.academicYear||student?.anneeAcademique||student?.anneeScolaire||student?.schoolYear||'');
            }
            async function loadRegistration() {
                const tbody=document.getElementById('registrationTable'); if(!tbody)return;
                tbody.innerHTML=`<tr><td colspan="9" class="text-center py-4"><div class="text-muted"><i class="fas fa-spinner fa-pulse fa-2x mb-3"></i><p>Loading registration records...</p></div></td></tr>`;
                try {
                    const [paymentsSnap, inscriptionsSnap, studentsSnap]=await Promise.all([registrationPaymentsRef.once('value'),pendingInscriptionsRef.once('value'),studentsRef.once('value')]);
                    const payments=[], inscriptions=[], students=new Map();
                    paymentsSnap.forEach(ch=>payments.push({id:ch.key,...(ch.val()||{})}));
                    inscriptionsSnap.forEach(ch=>inscriptions.push({id:ch.key,...(ch.val()||{})}));
                    studentsSnap.forEach(ch=>students.set(String(ch.key),{id:ch.key,...(ch.val()||{})}));
                    const paymentByStudent=new Map(); payments.forEach(p=>{const sid=String(p.studentId||p.studentID||'');if(sid)paymentByStudent.set(sid,p);});
                    const records=new Map(); const activeYear=comptableActiveAcademicYear();
                    inscriptions.forEach(insc=>{
                        const status=normalizeKey(insc.statut||insc.status||''); if(['rejete','rejected','cancelled','annule'].includes(status))return;
                        const sid=String(insc.studentId||insc.studentID||''); const student=students.get(sid)||{}; const payment=paymentByStudent.get(sid)||null;
                        const branchValue=payment?.branch||insc.branche||insc.branch||student.branch||'';
                        const sectionValue=payment?.section||insc.section||student.section||''; const classValue=payment?.studentClass||insc.classe||insc.niveau||getDuplicateStudentClass(student)||'';
                        if(!registrationRecordMatchesBranch(branchValue,sectionValue,classValue,selectedBranch))return;
                        const year=comptableRecordYear(payment,student)||comptableRecordYear(insc,student);
                        if(year && year!==activeYear)return;
                        const key=sid||`inscription:${insc.id}`; records.set(key,{...(payment||{}),id:payment?.id||`inscription-${insc.id}`,studentId:sid,studentName:payment?.studentName||student.name||student.fullName||insc.nomPrenom||insc.name||insc.fullName||'N/A',studentClass:getDuplicateStudentClass(student)||payment?.studentClass||insc.classe||insc.niveau||'N/A',branch:branchValue,academicYear:year||activeYear,reference:payment?.reference||student.reference||insc.codeUnique||`INS-${String(insc.id).slice(-8)}`,amount:payment?.amount??0,paymentDate:payment?.paymentDate||'',paymentMode:payment?.paymentMode||'',timestamp:payment?.timestamp||insc.timestamp||student.createdAt||0,registrationStatus:insc.statut||insc.status||'EnregistrÃ©',unpaid:!payment});
                    });
                    payments.forEach(payment=>{
                        const sid=String(payment.studentId||payment.studentID||''); const student=students.get(sid)||{}; const year=comptableRecordYear(payment,student); if(year && year!==activeYear)return;
                        const branchValue=payment.branch||student.branch||''; if(!registrationRecordMatchesBranch(branchValue,payment.section||student.section||'',payment.studentClass||getDuplicateStudentClass(student),selectedBranch))return;
                        const key=sid||`payment:${payment.id}`; if(!records.has(key))records.set(key,{...payment,studentName:payment.studentName||student.name||student.fullName||'N/A',studentClass:payment.studentClass||getDuplicateStudentClass(student)||'N/A',branch:branchValue,academicYear:year||activeYear,unpaid:false});
                    });
                    updateRegistrationTable(Array.from(records.values()).sort((a,b)=>(b.timestamp||0)-(a.timestamp||0)));
                } catch(error){console.error('Error loading registration records:',error);showToast('Error loading registration records.','error');tbody.innerHTML=`<tr><td colspan="9" class="text-center text-danger py-4">Unable to load registration records.</td></tr>`;}
            }
            ...
            function registrationRecordMatchesBranch(branch, section, studentClass, targetBranch) {
                ...
                // Firebase contains both short IDs and full labels such as
                // "EDEN FAMILY SCHOOL KACYIRU - MATERNELLE". Normalize them
                // to the real internal branch IDs before comparing.
                const normalizeBranchId = (value, sectionValue = '', classValue = '') => {
                    const b = String(value || '').trim().toLowerCase();
                    const s = String(sectionValue || '').trim().toLowerCase();
                    const c = String(classValue || '').trim().toLowerCase();
                    if (b.includes('kacyiru') && (b.includes('maternelle') || b.includes('nursery') || b.includes('creche'))) return b.includes('creche') ? 'kacyiru_creche' : 'kacyiru_maternelle';
                    if (b.includes('kacyiru')) return 'kacyiru';
                    if (b.includes('kimisagara')) return 'kimisagara';
                    if (b.includes('gisozi')) {
                        if (b.includes('maternelle') || b.includes('nursery')) return 'gisozi_maternelle';
                        if (b.includes('primaire') || b.includes('primary')) return 'gisozi_primaire';
                    }
                    if (b === 'gisozi') {
                        if (s.includes('maternelle') || s.includes('nursery') || c.startsWith('n')) return 'gisozi_maternelle';
                        if (s.includes('primaire') || s.includes('primary') || c.startsWith('p')) return 'gisozi_primaire';
                    }
                    if (['kacyiru','kacyiru_maternelle','kacyiru_creche','kimisagara','gisozi_maternelle','gisozi_primaire'].includes(b)) return b;
                    return b;
                };

                const normalizedTarget = normalizeBranchId(target, rawSection, rawClass);
                const normalizedRecord = normalizeBranchId(rawBranch, rawSection, rawClass);
                if (normalizedTarget && normalizedRecord && normalizedTarget === normalizedRecord) return true;

                // If the branch field is only "GISOZI", use section/class to resolve it.
                if (normalizedRecord === 'gisozi_maternelle' || normalizedRecord === 'gisozi_primaire') {
            ...
            function computeStudentDuplicateScore(candidate, existing) {
                ...
                // The name must carry most of the score: real duplicates often have
                // missing/changed parent fields, while the child's name remains very close.
                score += nameSim * 0.85;

                if (candidate.birthDate && existing.birthDate) {
                    if (candidate.birthDate === existing.birthDate) score += 8;
                }
                ...
                if (phoneA && phoneB && phoneA === phoneB) score += 7;
            ...
            function scanAllDuplicateGroupsFromRecords(records) {
                const branchStudents = (records || []).map(s => ({ ...s }));
                const groups = [];
                const used = new Set();
                for (let i = 0; i < branchStudents.length; i++) {
                    const a = branchStudents[i];
                    if (used.has(a.id)) continue;
                    const group = [a];
                    for (let j = i + 1; j < branchStudents.length; j++) {
                        const b = branchStudents[j];
                        if (used.has(b.id)) continue;
                        const score = computeStudentDuplicateScore(a, b).score;
                        const nameA = normalizeKey(getDuplicateStudentName(a));
                        const nameB = normalizeKey(getDuplicateStudentName(b));
                        const sameName = nameA && nameA === nameB;
                        // 70% catches realistic spelling/name variations such as
                        // HUDSON/HADSONNE or IHUMURE/IHUMURU without requiring exact fields.
                        if (score >= 70 || sameName) group.push(b);
                    }
                    if (group.length > 1) {
            ...
            function showExportAcademicYearModal(years) {
                return new Promise(resolve => {
                    ...
                    // Export must be explicitly filtered by BOTH branch and academic year.
                    // Keep the choices limited to the school years requested by the school.
                    const activeExportYear=comptableActiveAcademicYear(); const activeStart=Number(String(activeExportYear).slice(0,4))||new Date().getFullYear(); const yearOptions=[`${activeStart-1}-${activeStart}`,`${activeStart}-${activeStart+1}`,`${activeStart+1}-${activeStart+2}`];
                    const branchOptions = [
                    ...
                    const modalHtml = `<div class="modal fade" id="exportAcademicYearModal" tabindex="-1"><div class="modal-dialog modal-dialog-centered"><div class="modal-content export-year-modal-content"><div class="modal-header"><div><div class="export-year-kicker">EXPORT REPORT (PDF)</div><h5 class="modal-title">Filtrer l'export des inscriptions</h5><p class="export-year-subtitle">Choisissez obligatoirement la branche et l'année scolaire.</p></div><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body"><div class="mb-4"><label class="form-label fw-bold">Branche *</label><select id="exportRegistrationBranch" class="form-control-modern w-100"><option value="">— Sélectionner une branche —</option>${branchOptions.map(([v,l])=>`<option value="${v}">${l}</option>`).join('')}</select></div><div><label class="form-label fw-bold">Année scolaire *</label><select id="exportRegistrationYear" class="form-control-modern w-100"><option value="">— Sélectionner l'année scolaire —</option>${yearOptions.map(y=>`<option value="${y}">${y}</option>`).join('')}</select></div></div><div class="modal-footer"><button type="button" class="btn btn-light" data-bs-dismiss="modal">Annuler</button><button type="button" class="btn btn-primary" id="confirmExportAcademicYear" disabled>Exporter le rapport</button></div></div></div></div>`;
                    document.body.insertAdjacentHTML('beforeend', modalHtml);
                    const el=document.getElementById('exportAcademicYearModal');
                    const branchSelect=document.getElementById('exportRegistrationBranch');
                    const yearSelect=document.getElementById('exportRegistrationYear');
                    const confirm=document.getElementById('confirmExportAcademicYear');
                    const update=()=>{confirm.disabled=!(branchSelect.value && yearSelect.value);};
                    branchSelect.addEventListener('change',update); yearSelect.addEventListener('change',update);
                    el.addEventListener('hidden.bs.modal',()=>{el.remove();resolve(null)},{once:true});
                    confirm.addEventListener('click',()=>{const result={branch:branchSelect.value,year:yearSelect.value};bootstrap.Modal.getInstance(el).hide();resolve(result)},{once:true});
                    new bootstrap.Modal(el).show();
                });
            }

            async function buildCanonicalRegisteredStudents(selectedYear='ALL', exportBranch='ALL') {
                const [studentsSnap, registrationsSnap, pendingSnap] = await Promise.all([
                    studentsRef.once('value'), registrationPaymentsRef.once('value'), pendingInscriptionsRef.once('value')
                ]);
                const students=[], registrations=[], pending=[];
                studentsSnap.forEach(ch=>students.push({id:ch.key,...(ch.val()||{})}));
                registrationsSnap.forEach(ch=>registrations.push({id:ch.key,...(ch.val()||{})}));
                pendingSnap.forEach(ch=>{const r={id:ch.key,...(ch.val()||{})}; const st=normalizeKey(r.statut||r.status||''); if(!['rejete','rejected'].includes(st)) pending.push(r);});
                ...
                const dateValueOf = r => r?.paymentDate || r?.dateInscription || r?.enrollmentDate || r?.createdAt || r?.timestamp || '';
                const yearOf=r=>normalizeAcademicYear(r?.academicYear||r?.anneeAcademique||r?.anneeScolaire||r?.schoolYear||'') || normalizeAcademicYear(getAcademicYearForEnrollmentDate(dateValueOf(r)));
                const branchOf=r=>r?.branch||r?.branche||r?.section||r?.sectionScolaire||'';
                const canonical=[];
                const byId=new Map(); const byRef=new Map();
                const add=(record,source)=>{
                    const sid=String(record.studentId||record.studentID||'');
                    const ref=norm(record.reference||record.codeUnique||record.referenceId||'');
                    let row=sid?byId.get(sid):null;
                    if(!row && ref) row=byRef.get(ref);
                    if(!row){
                        row={studentId:sid||String(record.id||''),sourceRecords:{}}; canonical.push(row);
                    }
                    row.sourceRecords[source]=record;
                    if(sid){row.studentId=sid;byId.set(sid,row);}
                    if(ref)byRef.set(ref,row);
                    return row;
                };
                students.forEach(r=>add(r,'student'));
                registrations.forEach(r=>add(r,'registration'));
                pending.forEach(r=>add(r,'pending'));

                // Merge only strong identity matches for records without a shared studentId/reference.
                for(let i=0;i<canonical.length;i++) for(let j=i+1;j<canonical.length;j++){
                    const a=canonical[i],b=canonical[j]; if(a===b)continue;
                    const ar=a.sourceRecords.student||a.sourceRecords.registration||a.sourceRecords.pending||{};
                    const br=b.sourceRecords.student||b.sourceRecords.registration||b.sourceRecords.pending||{};
                    const an=norm(getDuplicateStudentName(ar)), bn=norm(getDuplicateStudentName(br));
                    const ad=norm(getDuplicateStudentBirthDate(ar)), bd=norm(getDuplicateStudentBirthDate(br));
                    const ap=normalizePhone(getDuplicateStudentPhone(ar)), bp=normalizePhone(getDuplicateStudentPhone(br));
                    const ac=norm(getDuplicateStudentClass(ar)), bc=norm(getDuplicateStudentClass(br));
                    if(an && an===bn && ((ad&&bd&&ad===bd)||(ap&&bp&&ap===bp))){
                        const target=a,other=b; target.sourceRecords={...other.sourceRecords,...target.sourceRecords};
                        if(!target.studentId)target.studentId=other.studentId; canonical.splice(j,1); j--;
                    } else if(computeStudentDuplicateScore(ar,br).score>=90 && ac && bc && ac===bc){
                        const target=a,other=b; target.sourceRecords={...other.sourceRecords,...target.sourceRecords}; canonical.splice(j,1); j--;
                    }
                }
                const selectedKey=norm(selectedYear); const rows=[];
                canonical.forEach(entity=>{
                    const student=entity.sourceRecords.student||{}; const payment=entity.sourceRecords.registration||{}; const insc=entity.sourceRecords.pending||{};
                    const recordYear=yearOf(student)||yearOf(insc)||yearOf(payment);
                    if(selectedYear!=='ALL' && recordYear && norm(recordYear)!==selectedKey)return;
                    if(selectedYear!=='ALL' && !recordYear)return;
                    const branch=student.branch||payment.branch||branchOf(insc)||'';
                    // Registration PDF export is intentionally independent of the branch
                    // currently selected in the accountant dashboard.
                    if(exportBranch !== 'ALL' && exportBranch && !registrationRecordMatchesBranch(branch,student.section||payment.section||insc.section, getDuplicateStudentClass(student)||getDuplicateStudentClass(payment)||getDuplicateStudentClass(insc), exportBranch))return;
                    const name=student.name||student.fullName||payment.studentName||insc.nomPrenom||insc.name||'';
                    if(!name)return;
                    rows.push({studentId:entity.studentId||student.id||'',reference:student.reference||payment.reference||insc.codeUnique||entity.studentId||'',name,className:getExportAssignedClass(student)||'N/A',branch:getBranchName(branch),academicYear:recordYear||selectedYear,bus:(student.transportEnrolled===true||norm(student.transportEnrolled)==='oui'||norm(insc.transport)==='oui')?'OUI':'NON',address:student.address||[insc.secteur,insc.umurenge,insc.akagari,insc.umudugudu].filter(Boolean).join(', ')||'N/A',parent:student.parent||insc.chefNomPrenom||'N/A',parentPhone:student.parentPhone||insc.chefTelephone||'N/A',spouse:student.spouseName||student.secondParent||insc.conjointNomPrenom||insc.secondParent||'N/A',spousePhone:student.spousePhone||student.secondParentPhone||insc.conjointTelephone||insc.secondParentPhone||'N/A',registrationStatus:insc.statut||insc.status||'Enregistré',paymentStatus:payment.id?'Payé / paiement enregistré':'Inscrit — paiement non enregistré'});
                });
                rows.sort((a,b)=>String(a.branch).localeCompare(String(b.branch),'fr')||String(a.className).localeCompare(String(b.className),'fr')||String(a.name).localeCompare(String(b.name),'fr'));
                console.info('[Registration Export] canonical registered students:',rows.length,{selectedYear,exportBranch});
                return rows;
            }

            async function exportRegistrationReportPDF() {
                const JsPDF=resolveJsPDF(); if(!JsPDF){showToast('PDF library non chargée','error');return;}
                try{
                    const exportSelection=await showExportAcademicYearModal(['2025-2026','2026-2027']);
                    if(!exportSelection)return;
                    const selectedAcademicYear=exportSelection.year;
                    const selectedExportBranch=exportSelection.branch;
                    // Export only the selected branch and selected academic year.
                    const rows=await buildCanonicalRegisteredStudents(selectedAcademicYear,selectedExportBranch);
                    const doc=new JsPDF({unit:'mm',format:'a4',orientation:'landscape'});
                    const titleBranch=getBranchName(selectedExportBranch); const today=new Date().toLocaleDateString('fr-FR');
                    doc.setFont('helvetica','bold');doc.setFontSize(16);doc.text('RAPPORT DES ÉLÈVES INSCRITS',14,13);
                    doc.setFont('helvetica','normal');doc.setFontSize(10);doc.text(`Branche : ${titleBranch} | Année scolaire : ${selectedAcademicYear} | Date : ${today}`,14,20);doc.text(`Total des élèves inscrits : ${rows.length}`,14,26);
                    const body=rows.map((r,i)=>[i+1,r.reference,r.name,r.className,r.branch,r.academicYear,r.bus,r.address,r.parent,r.parentPhone,r.spouse,r.spousePhone,r.paymentStatus]);
                    if(body.length)doc.autoTable({startY:31,head:[['N°','RÉFÉRENCE','ÉLÈVE','CLASSE','BRANCHE','ANNÉE','BUS','ADRESSE / RÉSIDENCE','PARENT / TUTEUR','TÉL. PARENT','2e PARENT','TÉL. 2e PARENT','STATUT PAIEMENT']],body,styles:{fontSize:6.3,cellPadding:1.5,overflow:'linebreak',valign:'middle'},headStyles:{fontSize:6.3,fontStyle:'bold'},margin:{left:8,right:8,top:31,bottom:14},didDrawPage:data=>{doc.setFontSize(7);doc.text(`Rapport des élèves inscrits - page ${data.pageNumber}`,8,203);}});else doc.setFontSize(11),doc.text('Aucun élève inscrit trouvé pour cette sélection.',14,38);
                    const busCount=rows.filter(r=>r.bus==='OUI').length; const finalY=doc.lastAutoTable?doc.lastAutoTable.finalY:42; const summaryY=finalY+8;if(summaryY>190)doc.addPage();doc.setFont('helvetica','bold');doc.setFontSize(9);doc.text(`Total : ${rows.length} élève(s) | Bus : ${busCount} | Sans bus : ${rows.length-busCount}`,8,summaryY>190?20:summaryY);
                    const safeBranch=String(selectedExportBranch||'branche').replace(/[^a-z0-9_-]/gi,'_');const safeYear=String(selectedAcademicYear).replace(/[^a-z0-9_-]/gi,'_');doc.save(`Rapport_Eleves_Inscrits_${safeBranch}_${safeYear}.pdf`);showToast(`Rapport PDF généré : ${rows.length} élève(s)`, 'success');
                }catch(error){console.error('Erreur export rapport inscription:',error);showToast("Erreur lors de l’export : "+(error.message||error),'error');}
            }
            ...
            async function deletePayrollHistoryRecord(recordId) {
                ...
                try {
                    await moveToTrash(payrollHistoryRef, recordId, {source:'payrollHistory', reason:'Suppression d’un historique de payroll'});
                    showToast('Payroll history record moved to trash.', 'success');
                } catch (error) {
            ...
            async function clearPayrollHistoryForCurrentBranch() {
                ...
                try {
                    await Promise.all(branchRecords.map(record => moveToTrash(payrollHistoryRef, record.id, {source:'payrollHistory', reason:'Suppression groupée de l’historique de payroll'})));
                    showToast('Payroll history moved to trash for current branch.', 'success');
                } catch (error) {
            ...
            async function deleteWorker(workerId) {
                try {
                    ...
                    await moveToTrash(worksRef, workerId, {source:'workers', reason:'Suppression d’un travailleur', label:worker.name || worker.fullName || workerId});
                    showToast('Worker moved to trash successfully!', 'success');
            ...
            async function deleteDebt(debtId) {
                ...
                try {
                    await moveToTrash(debtsRef, debtId, {source:'debts', reason:'Suppression d’une dette'});
                    showToast('Debt moved to trash successfully!', 'success');
                    loadCurrentMonthDebts();
            ...
            async function deleteStudent(studentId) {
                try {
                    ...
                    for (const ref of paymentRefs) {
                        ...
                        if (!snapshot.exists()) continue;
                        const linked = [];
                        snapshot.forEach(child => linked.push(child.key));
                        for (const key of linked) {
                            const check = await ref.child(key).once('value');
                            if (check.exists()) await moveToTrash(ref, key, {source:'student-linked-data', reason:`Suppression de l’élève ${student.name || studentId}`, label:check.val()?.reference || check.val()?.studentName || key});
                        }
                    }

                    await moveToTrash(studentsRef, studentId, {source:'students', reason:'Suppression d’un élève', label:student.name || student.fullName || studentId});
                    showToast(currentLanguage === 'fr' ? 'Élève déplacé dans la corbeille !' : 'Student moved to trash!', 'success');
            ...
            // Export ALL registered students to Excel — independent of the accountant's
            // current branch, class, age, or table filters.
            function exportStudentsExcel() {
                try {
                    const allStudents = Array.isArray(students) ? [...students] : [];
                    const filteredStudents = allStudents;
                    
                    // Préparer les données pour l'export de TOUS les élèves
                    ...
                    showToast(`Tous les élèves (${filteredStudents.length}) ont été exportés vers Excel.`, 'success');
                } catch (error) {
            ...
            async function deleteFeePayment(paymentId) {
                try {
                    await moveToTrash(feesPaymentsRef, paymentId, {source:'feesPayments', reason:'Suppression d’un paiement de frais'});
                    showToast('Fee payment moved to trash successfully!', 'success');
                    loadFees();
                } catch (error) {
            ...
            async function deleteStudentFee(feeId) {
                try {
                    await moveToTrash(studentFeesRef, feeId, {source:'studentFees', reason:'Suppression d’un paiement élève'});
                    showToast('Payment moved to trash successfully!', 'success');
            ...
            async function deleteSelectedDuplicatePayments() {
                ...
                try {
                    ...
                    await Promise.all(selectedIds.map(id => moveToTrash(studentFeesRef, id, {source:'duplicate-payments', reason:'Doublon de paiement sélectionné dans le système de détection des doublons'})));
            ...
            let registrationDuplicateGroupsState = [];

            function escapeDuplicateHtml(value) {
                return String(value ?? '').replace(/[&<>'"]/g, ch => ({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[ch]));
            }

            function getDuplicateStudentName(rec) {
                return rec?.name || rec?.fullName || rec?.studentName || rec?.nomPrenom || 'Élève sans nom';
            }

            function getDuplicateStudentClass(rec) {
                // For reports and duplicate analysis, always prefer the CLASS ACTUALLY
                // ASSIGNED after confirmation. Requested class / application level must
                // never replace the confirmed class.
                return rec?.assignedClass || rec?.classAssigned || rec?.classeAffectee || rec?.classeAffectée || rec?.assigned_class || rec?.class || rec?.classes || rec?.studentClass || '';
            }

            function getDuplicateStudentPhone(rec) {
                return rec?.parentPhone || rec?.chefTelephone || rec?.parent || '';
            }

            function getExportAssignedClass(student) {
                // The registration report must show the class assigned at confirmation,
                // never the class requested on the application/payment.
                if (!student) return '';
                return student.assignedClass || student.classAssigned || student.classeAffectee || student.classeAffectée || student.assigned_class || student.class || student.classes || '';
            }

            function getDuplicateStudentBirthDate(rec) {
                return rec?.birthDate || rec?.studentBirthDate || rec?.dateNaissance || '';
            }
            ...
            function openRegistrationDuplicateModal() {
                ...
                registrationDuplicateGroupsState = [];
                summary.className = 'alert alert-secondary mb-3';
                summary.innerHTML = '<i class="fas fa-shield-alt me-2"></i>Le système va rechercher les doublons. <strong>Aucune suppression ne sera faite automatiquement.</strong> Pour chaque groupe, vous devrez choisir l\'élève correct ou indiquer que les deux sont différents.';
                tableBody.innerHTML = '<tr><td colspan="9" class="text-center text-muted py-4"><i class="fas fa-search me-2"></i>Appuyez sur <strong>Analyse</strong> pour démarrer.</td></tr>';
                ...
                if (countEl) countEl.textContent = '0 groupe(s) confirmé(s)';
                if (studentsEl) studentsEl.textContent = 'Décisions : aucune';
                if (deleteBtn) { deleteBtn.disabled = true; deleteBtn.innerHTML = '<i class="fas fa-trash me-2"></i>Nettoyer les doublons confirmés'; }
                if (!pendingInscriptionsData.length && typeof loadPendingInscriptions === 'function') {
                    loadPendingInscriptions().catch(error => console.error('Error loading pending inscriptions:', error));
                }
            ...
            function collectRegistrationDuplicateGroups() {
                // Analyse the THREE registration sources together. Payment records that
                // share the same studentId are payment history, not duplicate students.
                const records = [];
                const addRecords = (source, list, branchMatcher) => {
                    (list || []).forEach(r => {
                        if (!branchMatcher(r)) return;
                        const id = r.id || '';
                        records.push({ source, id: String(id), record: r, timestamp: r.timestamp || r.createdAt || 0 });
                    });
                };
                const branchMatcher = (record, source) => {
                    if (!selectedBranch || selectedBranch === 'all') return true;
                    if (source === 'pending') return inscriptionMatchesSelectedBranch(record);
                    return registrationRecordMatchesBranch(record?.branch, record?.section, getDuplicateStudentClass(record), selectedBranch);
                };
                // Pass the source explicitly: pending inscriptions do not have a
                // `source` property on their Firebase record.
                const addRecordsSafe = (source, list) => addRecords(source, list, record => branchMatcher(record, source));
                addRecordsSafe('student', window.__registrationDuplicateStudents || []);
                addRecordsSafe('registration', registrationPayments || []);
                addRecordsSafe('pending', pendingInscriptionsData || []);

                const canonical = records.map(item => ({
                    ...item,
                    studentId: String(item.record?.studentId || item.record?.studentID || item.record?.student_id || ''),
                    nameKey: normalizeKey(getDuplicateStudentName(item.record)),
                    classKey: normalizeKey(getDuplicateStudentClass(item.record)),
                    dobKey: normalizeKey(getDuplicateStudentBirthDate(item.record)),
                    phoneKey: normalizePhone(getDuplicateStudentPhone(item.record)),
                    referenceKey: normalizeKey(item.record?.reference || item.record?.codeUnique || item.record?.referenceId || '')
                }));

                const parent = new Map(canonical.map((_, i) => [i, i]));
                ...
                for (let i=0;i<canonical.length;i++) for (let j=i+1;j<canonical.length;j++) {
                    ...
                    // Same database student = one student. Multiple payments must never form a duplicate group.
                    if (a.studentId && b.studentId && a.studentId === b.studentId) continue;
                    if (a.source === 'registration' && b.source === 'registration' && a.id === b.id) continue;
                    if (a.source === 'pending' && b.source === 'pending' && a.id === b.id) continue;
                    if (a.source === 'student' && b.source === 'student' && a.id === b.id) continue;

                    const sameRef = a.referenceKey && b.referenceKey && a.referenceKey === b.referenceKey;
                    const sameName = a.nameKey && b.nameKey && a.nameKey === b.nameKey;
                    const samePhone = a.phoneKey && b.phoneKey && a.phoneKey === b.phoneKey;
                    const sameDob = a.dobKey && b.dobKey && a.dobKey === b.dobKey;
                    const sameClass = a.classKey && b.classKey && a.classKey === b.classKey;
                    const score = computeStudentDuplicateScore(a.record, b.record).score;

                    // Strong identity matches, or a high similarity score with supporting data.
                    const strong = sameRef || (sameName && (sameDob || samePhone || sameClass)) ||
                        (score >= 82 && (sameDob || samePhone || sameClass));
                    if (strong) union(i,j);
                }

                const grouped = new Map();
                canonical.forEach((item,i) => { const root=find(i); if(!grouped.has(root)) grouped.set(root,[]); grouped.get(root).push(item); });

                // Registration-payment rows can point to the real student with studentId.
                // Add that student record to the duplicate group so the accountant can
                // select the actual student to keep. Payment history itself remains intact.
                const studentById = new Map();
                (window.__registrationDuplicateStudents || []).forEach(student => {
                    const sid = String(student.id || '');
                    if (sid) studentById.set(sid, student);
                });
                ...
                return [...grouped.values()]
                    ...
                    .map(group => {
                        ...
                        const linkedIds = new Set(group
                            .filter(item => item.source !== 'student')
                            .map(item => String(item.studentId || item.record?.studentId || item.record?.studentID || item.record?.student_id || ''))
                            .filter(Boolean));
                        linkedIds.forEach(studentId => {
                            const student = studentById.get(studentId);
                            if (!student) return;
                            const already = enriched.some(item => item.source === 'student' && String(item.id) === studentId);
                            if (!already) enriched.unshift({
                                source: 'student', id: studentId, record: student, timestamp: student.timestamp || 0
                            });
                        });
                        return enriched;
                    })
                    // A group containing several registration payments for ONE student
                    // is payment history, not a duplicate student group.
                    .filter(group => {
                        const ids = new Set(group.filter(item => item.source === 'student').map(item => String(item.id || '')).filter(Boolean));
                        return ids.size >= 2 || !ids.size;
                    });
            }
...
async function analyseRegistrationDuplicates() {
                ...
                registrationDuplicateCandidates = [];
                registrationDuplicateGroupsState = [];
                tableBody.innerHTML = '<tr><td colspan="9" class="text-center py-4"><i class="fas fa-spinner fa-spin me-2"></i>Analyse complète des inscriptions...</td></tr>';
                try {
                    const [studentSnapshot, registrationSnapshot, pendingSnapshot] = await Promise.all([
                        studentsRef.once('value'), registrationPaymentsRef.once('value'), pendingInscriptionsRef.once('value')
                    ]);
                    const students = [], registrations = [], pending = [];
                    studentSnapshot.forEach(ch => students.push({id:ch.key,...(ch.val()||{})}));
                    registrationSnapshot.forEach(ch => registrations.push({id:ch.key,...(ch.val()||{})}));
                    pendingSnapshot.forEach(ch => {
                        const r={id:ch.key,...(ch.val()||{})};
                        const status=normalizeKey(r.statut||r.status||'');
                        if(!['rejete','rejected'].includes(status)) pending.push(r);
                    });
                    window.__registrationDuplicateStudents = students;
                    const groups = collectRegistrationDuplicateGroups();
                    window.__registrationDuplicateStudents = [];
                    if (!groups.length) {
                        summary.className='alert alert-success mb-3';
                        summary.innerHTML='<i class="fas fa-check-circle me-2"></i><strong>Aucun doublon probable détecté.</strong> Les paiements multiples du même élève ne sont pas considérés comme des doublons.';
                        tableBody.innerHTML='<tr><td colspan="9" class="text-center text-success py-4">Aucun doublon détecté.</td></tr>';
                        if(deleteBtn) deleteBtn.disabled=true;
                        updateRegistrationDuplicateSelectionSummary();
                        return;
                    }
                    registrationDuplicateGroupsState=groups.map((items,index)=>({source:'mixed',items,index,decision:null,deleteIds:new Set()}));
                    const totalRows=groups.reduce((n,g)=>n+g.length,0);
                    summary.className='alert alert-warning mb-3';
                    summary.innerHTML=`<strong>${groups.length}</strong> groupe(s) de doublons probable(s), <strong>${totalRows}</strong> enregistrements à vérifier. <strong>Aucune suppression automatique.</strong>`;
                    const rows=[];
                    registrationDuplicateGroupsState.forEach((group,gi)=>{
                        const first=group.items[0]?.record||{};
                        rows.push(`<tr class="table-light"><td colspan="9"><strong>GROUPE ${gi+1}</strong> — ${escapeDuplicateHtml(getDuplicateStudentName(first))}<br><small class="text-danger"><i class="fas fa-check-square me-1"></i>Cochez les élèves que vous voulez <strong>EFFACER</strong>. L'élève non coché sera conservé. Vous devez toujours garder au moins un élève.</small></td></tr>`);
                        const studentItems = group.items.filter(it => it.source === 'student');
                        group.items.forEach((it,idx)=>{
                            const rec=it.record||{}; const sid=it.id||rec.id||rec.studentId||'';
                            const score=idx===0?'Référence':`${computeStudentDuplicateScore(rec,first).score}%`;
                            const sourceLabel=it.source==='student'?'Élève':it.source==='registration'?'Paiement inscription':'Demande inscription';
                            const value=`${it.source}::${sid}`;
                            const selector=it.source==='student' ? `<label style="display:flex;align-items:center;justify-content:center;gap:6px;min-width:52px;cursor:pointer;"><input type="checkbox" class="form-check-input registration-duplicate-delete" value="${escapeDuplicateHtml(sid)}" data-group="${gi}" onchange="toggleRegistrationDuplicateDelete(${gi}, this.value, this.checked)" title="Cochez pour EFFACER cet élève" aria-label="Effacer cet élève" style="appearance:auto !important;-webkit-appearance:checkbox !important;display:inline-block !important;width:24px;height:24px;margin:0;cursor:pointer;accent-color:#dc3545;"><span style="font-size:10px;font-weight:700;color:#dc3545;">EFFACER</span></label>` : '<span class="text-muted">—</span>';
                            rows.push(`<tr><td>${selector}</td><td><span class="badge bg-secondary">${sourceLabel}</span></td><td><strong>${escapeDuplicateHtml(getDuplicateStudentName(rec))}</strong><br><small>${escapeDuplicateHtml(rec.reference||rec.codeUnique||rec.referenceId||'')}</small></td><td>${escapeDuplicateHtml(score)}</td><td>${escapeDuplicateHtml(getDuplicateStudentClass(rec))}</td><td>${escapeDuplicateHtml(getDuplicateStudentBirthDate(rec)||'N/A')}</td><td>${escapeDuplicateHtml(rec.parent||rec.chefNomPrenom||rec.parentName||'')}</td><td>${escapeDuplicateHtml(getDuplicateStudentPhone(rec))}</td><td>${escapeDuplicateHtml(rec.enrollmentDate||rec.paymentDate||rec.createdAt||'')}</td></tr>`);
                        });
                        rows.push(`<tr><td colspan="9" class="bg-light"><strong>Suppression — groupe ${gi+1} :</strong> <span class="text-muted">les élèves cochés seront envoyés dans la Corbeille. Laissez au moins une fiche élève non cochée.</span></td></tr>`);
                    });
                    tableBody.innerHTML=rows.join('');
                    updateRegistrationDuplicateSelectionSummary();
                } catch(error) {
                    console.error('Duplicate analysis error:',error);
                    summary.className='alert alert-danger mb-3'; summary.textContent="Erreur pendant l’analyse des doublons.";
                    tableBody.innerHTML='<tr><td colspan="9" class="text-center text-danger py-4">Impossible d’analyser les doublons.</td></tr>';
                    if(deleteBtn) deleteBtn.disabled=true;
                } finally { window.__registrationDuplicateStudents=[]; }
            }

function toggleRegistrationDuplicateDelete(groupIndex, studentId, checked) {
                const group = registrationDuplicateGroupsState[groupIndex];
                if (!group) return;
                if (!group.deleteIds) group.deleteIds = new Set();
                const id = String(studentId || '');
                if (!id) return;
                if (checked) group.deleteIds.add(id); else group.deleteIds.delete(id);
                // Keep the old decision field synchronized for compatibility with existing code.
                const students = group.items.filter(it => it.source === 'student');
                const remaining = students.filter(it => !group.deleteIds.has(String(it.id || it.record?.id || it.record?.studentId || '')));
                group.decision = remaining.length === 1 ? `student::${remaining[0].id || remaining[0].record?.id || ''}` : (group.deleteIds.size ? 'DELETE_SELECTED' : null);
                updateRegistrationDuplicateSelectionSummary();
            }

            function setRegistrationDuplicateDecision(groupIndex, value) {
                if (!registrationDuplicateGroupsState[groupIndex]) return;
                registrationDuplicateGroupsState[groupIndex].decision = value || null;
                const select = document.querySelector(`.registration-duplicate-decision[onchange="setRegistrationDuplicateDecision(${groupIndex}, this.value)"]`);
                if (select && select.value !== value) select.value = value || '';
                document.querySelectorAll(`input[name="registrationDupGroup_${groupIndex}"]`).forEach(r => r.checked = r.value === value);
                updateRegistrationDuplicateSelectionSummary();
            }

            function selectRecommendedRegistrationDuplicates() {
                registrationDuplicateGroupsState.forEach(group => {
                    const students = group.items.filter(it => it.source === 'student');
                    if (students.length <= 1) return;
                    if (!group.deleteIds) group.deleteIds = new Set();
                    // Recommended: keep the first/reference student and select all other copies.
                    students.slice(1).forEach(it => group.deleteIds.add(String(it.id || it.record?.id || it.record?.studentId || '')));
                    group.decision = 'DELETE_SELECTED';
                });
                document.querySelectorAll('.registration-duplicate-delete').forEach(cb => {
                    const gi = Number(cb.dataset.group);
                    const group = registrationDuplicateGroupsState[gi];
                    cb.checked = !!group?.deleteIds?.has(String(cb.value));
                });
                updateRegistrationDuplicateSelectionSummary();
            }

            function clearRegistrationDuplicateSelection() {
                registrationDuplicateGroupsState.forEach(group => { group.decision = null; group.deleteIds = new Set(); });
                document.querySelectorAll('.registration-duplicate-delete').forEach(cb => cb.checked = false);
                updateRegistrationDuplicateSelectionSummary();
            }

            function updateRegistrationDuplicateSelectionSummary() {
                const groups = registrationDuplicateGroupsState || [];
                const selected = groups.flatMap(g => Array.from(g.deleteIds || []));
                const invalid = groups.some(g => {
                    const students = g.items.filter(it => it.source === 'student');
                    return (g.deleteIds?.size || 0) >= students.length;
                });
                const countEl = document.getElementById('registrationDuplicateSelectedCount');
                const studentsEl = document.getElementById('registrationDuplicateSelectedStudents');
                const deleteBtn = document.getElementById('deleteRegistrationDuplicatesBtn');
                if (countEl) countEl.textContent = `${selected.length} élève(s) sélectionné(s) pour suppression`;
                if (studentsEl) {
                    const names = groups.flatMap(g => g.items.filter(it => it.source === 'student' && (g.deleteIds || new Set()).has(String(it.id || it.record?.id || it.record?.studentId || ''))).map(it => getDuplicateStudentName(it.record))).filter(Boolean);
                    studentsEl.textContent = names.length ? `À supprimer : ${names.join(', ')}` : 'À supprimer : aucun élève sélectionné';
                    if (invalid) studentsEl.textContent += ' — ERREUR : gardez au moins un élève par groupe.';
                }
                if (deleteBtn) deleteBtn.disabled = selected.length === 0 || invalid;
            }

            async function reassignStudentData(oldStudentId, newStudentId) {
                if (!oldStudentId || !newStudentId || oldStudentId === newStudentId) return;
                const refs = [studentFeesRef, feesPaymentsRef, coachingPaymentsRef, transportPaymentsRef, registrationPaymentsRef];
                for (const ref of refs) {
                    const snap = await ref.orderByChild('studentId').equalTo(oldStudentId).once('value');
                    if (!snap.exists()) continue;
                    const updates = {};
                    snap.forEach(child => updates[child.key + '/studentId'] = newStudentId);
            ...
            async function deleteSelectedRegistrationDuplicates() {
                const groups = registrationDuplicateGroupsState || [];
                const selectedGroups = groups.filter(g => (g.deleteIds?.size || 0) > 0);
                if (!selectedGroups.length) { showToast('Cochez au moins un élève à EFFACER.', 'error'); return; }
                const invalid = selectedGroups.find(g => {
                    const students = g.items.filter(it => it.source === 'student');
                    return !students.length || (g.deleteIds.size >= students.length);
                });
                if (invalid) { showToast('Vous devez laisser au moins un élève dans chaque groupe.', 'error'); return; }
                const totalSelected = selectedGroups.reduce((n,g)=>n+g.deleteIds.size,0);
                const confirmed = confirm(`Confirmer la suppression de ${totalSelected} élève(s) sélectionné(s) ? Les élèves seront envoyés dans la Corbeille et pourront être restaurés. Les paiements et leur historique seront conservés.`);
                if (!confirmed) return;
                const deleteBtn=document.getElementById('deleteRegistrationDuplicatesBtn'); const originalHtml=deleteBtn?.innerHTML||'';
                try {
                    if(deleteBtn){deleteBtn.disabled=true;deleteBtn.innerHTML='<i class="fas fa-spinner fa-spin me-2"></i>Suppression sécurisée...';}
                    let removedStudents=0, reassigned=0, removedPending=0;
                    for(const group of selectedGroups){
                        const students=group.items.filter(it=>it.source==='student');
                        const keeper=students.find(it=>!group.deleteIds.has(String(it.id||it.record?.id||it.record?.studentId||'')));
                        if(!keeper) continue;
                        const keeperId=String(keeper.id||keeper.record?.id||'');
                        for(const item of group.items){
                            const itemId=String(item.id||item.record?.id||item.record?.studentId||'');
                            if(item.source==='student' && group.deleteIds.has(itemId) && itemId!==keeperId){
                                await reassignStudentData(itemId,keeperId);
                                await moveToTrash(studentsRef,itemId,{source:'registration-duplicates',reason:`Doublon d'inscription — élève conservé : ${getDuplicateStudentName(keeper.record)}`,label:getDuplicateStudentName(item.record)});
                                removedStudents++; reassigned++;
                            } else if(item.source==='pending' && keeperId){
                                await moveToTrash(pendingInscriptionsRef,itemId,{source:'registration-duplicates',reason:`Demande d'inscription en double — élève conservé : ${getDuplicateStudentName(keeper.record)}`,label:getDuplicateStudentName(item.record)});
                                removedPending++;
                            }
                        }
                    }
                    showToast(`${removedStudents} élève(s) supprimé(s) et placé(s) dans la Corbeille. ${reassigned} historique(s) financier(s) rattaché(s). ${removedPending} demande(s) en double placée(s) dans la Corbeille.`, 'success');
                    registrationDuplicateGroupsState=[]; registrationDuplicateCandidates=[];
                    if(typeof loadRegistration==='function') await loadRegistration();
                    if(typeof applyStudentFilters==='function') applyStudentFilters();
                    const summary=document.getElementById('registrationDuplicateSummary');
                    if(summary){summary.className='alert alert-success mb-3';summary.innerHTML="<i class=\"fas fa-check-circle me-2\"></i>Suppression terminée. Les élèves sélectionnés sont dans la Corbeille. L’historique des paiements est conservé.";}
                    const tableBody=document.getElementById('registrationDuplicateTableBody');
                    if(tableBody)tableBody.innerHTML='<tr><td colspan="9" class="text-center text-success py-4">Suppression terminée. Relancez Analyse pour vérifier.</td></tr>';
                    updateRegistrationDuplicateSelectionSummary();
                } catch(error){ console.error('Error cleaning registration duplicates:',error); showToast('Erreur pendant la suppression sécurisée : '+(error.message||error),'error'); }
                finally{if(deleteBtn){deleteBtn.disabled=true;deleteBtn.innerHTML=originalHtml||'<i class="fas fa-trash me-2"></i>Supprimer les Doublons Sélectionnés';}}
            }
            ...
            async function removeExpense(expenseId) {
                ...
                try {
                    await moveToTrash(dailyExpensesRef, expenseId, {source:'dailyExpenses', reason:'Suppression d’une dépense journalière'});
            ...
            async function deleteCoachingPayment(paymentId) {
                ...
                try {
                    await moveToTrash(coachingPaymentsRef, paymentId, {source:'coachingPayments', reason:'Suppression d’un paiement de coaching'});
                    showToast('Coaching payment moved to trash successfully!', 'success');
                    loadCoaching();
                    recalculateExpectedIncome();
                } catch (error) {
