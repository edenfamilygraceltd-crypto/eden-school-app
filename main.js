        ...
        async function loadClassesFromFirebase() {
            try {
                ...
                if (classesSnapshot.exists()) {
                    const classesObj = classesSnapshot.val();
                    classesData = [];
                    Object.entries(classesObj).forEach(([key, value]) => {
                        classesData.push({
                            id: key,
                            name: value.name || value.niveau || value.label || key,
                            level: value.level || value.niveau || '',
                            branch: value.branch || value.branche || ''
                        });
                    });
